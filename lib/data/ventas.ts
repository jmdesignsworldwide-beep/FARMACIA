...
export async function getProductosVendibles(): Promise<ProductoVendible[]> {
  ...
  const [{ data: productos }, { data: lotes }] = await Promise.all([
    supabase
      ...
      .select(
        "id, nombre_comercial, nombre_generico, concentracion, presentacion, categoria, codigo_barras, precio_venta, controlado, requiere_receta, itbis_gravado, stock_total, stock_minimo, unidades_por_caja, vende_caja, precio_caja, vende_unidad",
      )
      .eq("activo", true)
      .gt("stock_total", 0)
